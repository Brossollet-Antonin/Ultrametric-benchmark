...
for T in temperature:
    for mem in memory:
        ...
        path = "C:/Users/Antonin/Documents/Documents/ENS 2A/Stage M1/Results/Memory/"+dataset+"/memory"+str(mem)+" T"+str(T)
        ...
        original = pd.DataFrame(dict(nbr_ex=original_accuracy[:,1], acc=original_accuracy[:,0], 
                                     Mode=modeo, Memory=(np.ones(size, dtype=int)*mem)))
        ...
        shuffle = pd.DataFrame(dict(nbr_ex=shuffle_accuracy[:,1], acc=shuffle_accuracy[:,0], 
                                    Mode=modes, Memory=(np.ones(size, dtype=int)*mem)))
...
palette = sns.color_palette("GnBu_d", len(memory))
palette.reverse()

fig, ax = plt.subplots(figsize=(9,9))
final_plot = sns.lineplot(x="nbr_ex", y='acc', data=totaldata, ci=None, 
                         hue="Memory", hue_norm=LogNorm(), style="Mode", legend="full", palette=palette)
