...
def learning_ER(net, training, train_data_rates, mem_sz, batch_sz, lr, momentum, training_range):
    if training.training_type=="temporal correlation" or training.training_type=="spatial correlation" or training.training_type=="random" or training.training_type=="uniform":
        ...
        train_data =  train_data_rates[0] if training.training_type=="temporal correlation" or training.training_type=="spatial correlation" or training.training_type=="uniform" else train_data_rates
...
# Botch solution just to try to see if equivalent to memory size = 0. Should be faster than memory_size=0, even though probably not significantly
    
def learning_naive(net, training, train_data_prev, batch_sz, lr, momentum):
    running_loss = 0.0
    n = 0
    train_data_rates = train_data_prev  #Stock rates (if not a random process) and data for training
    train_data =  train_data_rates[0] if training.training_type=="temporal correlation" or training.training_type=="spatial correlation" else train_data_rates
    rates = train_data_rates[1] if training.training_type=="temporal correlation" or training.training_type=="spatial correlation" else 'Random process, no rates involved'
    train_sequence = train_data_rates[2] if training.training_type=="temporal correlation" or training.training_type=="spatial correlation" else 'Random process, no sequence involved'
    length_data = len(train_data)
    # Define mini-batches of size training.task_sz_nbr and SGD and update the memory for each of them
    while n + training.task_sz_nbr < length_data:
        mini_batch = deepcopy(train_data[n])
        for data in train_data[n+1:n+training.task_sz_nbr]:
            mini_batch[0] = torch.cat((mini_batch[0], data[0]))
            mini_batch[1] = torch.cat((mini_batch[1], data[1]))
        # Perform SGD on the mini_batch    
        running_loss += train.mem_SGD(net, mini_batch, lr, momentum, training.device)
        n += training.task_sz_nbr
        if n % (100*training.task_sz_nbr) == 99*training.task_sz_nbr:
            print("[%d] loss: %.4f" % (n//training.task_sz_nbr + 1, running_loss/100))
            running_loss = 0.0
