...
class SequenceShuffler:
    ...
    def shuffle_sequence(self, net, training, control_data, mem_sz, batch_sz, lr, momentum, training_range):
    ...
    def shuffle_block(self, sequence, block_size):
        ...
        for i in block_indices:
            control_data_block += copied_sequence[i*block_size:(i+1)*block_size]
        return control_data_block
    

    def shuffle_block_stef(self, sequence, block_size):
        # Return a sequence shuffled by block using the different method from Stefano and Marcus for the shuffle 
        nb = len(sequence)//block_size    #number of blocks
        ns = 10*nb      #number of shuffles that will be done
        sequence_shuffle = deepcopy(sequence)
        for i in range(ns):
            ia = random.randint(0, nb-1)*block_size
            ib = random.randint(0, nb-1)*block_size
            while ia == ib:
                ib = random.randint(0, nb-1)*block_size
            databuffer = sequence_shuffle[ia:ia+block_size]
            sequence_shuffle[ia:ia+block_size] = sequence_shuffle[ib:ib+block_size]
            sequence_shuffle[ib:ib+block_size] = databuffer
        return sequence_shuffle


    def shuffle_block_partial(self, train_dataset, block_size, end): 
