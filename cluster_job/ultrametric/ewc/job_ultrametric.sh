...
# seq_types="ultrametric random_blocks2 uniform" split_length=1000 batch_sz=100 lambda=200 fisher_n=200
    ...
    do
      ...
      do
        for (( value = 1; value <= $n_reps; value++ ))
        	do
        	  sbatch individualjob_ultrametric.sh ${tree_depth} ${hidden_size} ${split_length} ${temperature} ${seqtype} ${sl} ${flip_rate} ${batch_sz} ${lambda} ${fisher_n} ${block_sizes[*]} 
        	  sleep 1
        	done
      done
    done
