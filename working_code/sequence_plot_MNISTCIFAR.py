# -*- coding: utf-8 -*-
"""
Created on Fri Jun 21 14:28:17 2019
...
plt.title("Accuracy : %.2f %% \n Memory size: %d Minibatches size: %d Epoch: %d \n Energy step: %d Temperature: %.3f Branching: %d" % (original_accuracy[-1,0], memory_sz, minibatches, epoch, step, T, tree_branching))
plt.legend()
...
plt.title("Accuracy : %.2f %% \n Memory size: %d Minibatches size: %d Epoch: %d \n Energy step: %d Temperature: %.3f Branching : %d" % (shuffle_accuracy[-1,0], memory_sz, minibatches, epoch, step, T, tree_branching))
plt.legend()
...
plt.ylim(0, 105)
plt.plot(x, original_accuracy[:, 0], label='Original')
plt.plot(x, shuffle_accuracy[:, 0], label='Shuffle')
